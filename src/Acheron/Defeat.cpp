...
namespace Acheron
{
	...
	void Defeat::DefeatActor(RE::Actor* a_victim)
	{
		...
		Victims.emplace(a_victim->GetFormID(), std::make_shared<VictimData>(data));
	...
	void Defeat::RescueActor(RE::Actor* a_victim, bool undo_pacify)
	{
		...
		if (undo_pacify) {
			UndoPacifyLocked(a_victim, lock);
		}
	...
	void Defeat::PacifyUnsafe(RE::Actor* a_victim, [[maybe_unused]] std::unique_lock<std::shared_mutex>& lock)
	{
	...
	void Defeat::UndoPacifyLocked(RE::Actor* a_victim, [[maybe_unused]] std::unique_lock<std::shared_mutex>& lock)
	{
		assert(lock.owns_lock());
